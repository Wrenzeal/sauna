          ...
          <nav className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto px-1 [scrollbar-width:none]" aria-label="主导航">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  {active ? <motion.span layoutId="top-nav-active" className="absolute inset-0 rounded-full bg-[var(--sauna-soft-strong)]" transition={{ type: "spring", stiffness: 360, damping: 32 }} /> : null}
                  <Icon className="relative" size={17} weight={active ? "duotone" : "regular"} />
                  <span className="relative hidden md:inline">{item.label}</span>
                  {active ? <span className="absolute inset-x-4 -bottom-1 h-px bg-[var(--sauna-accent)]" /> : null}
                </Link>
              );
            })}
          </nav>
