...
function assistantStateLabel(streamStatus: string, messages: Message[], sessionId: string) {
  ...
  const latestAssistant = [...messages].reverse().find((message) => message.role === "assistant" && message.session_id === sessionId);
  ...
  if (latestAssistant?.status === "partial" || latestAssistant?.status === "pending") {
    return latestAssistant.content?.trim() ? "正在输出" : "正在思考";
  }
    ...
    <span className="inline-flex items-center gap-1" aria-hidden="true">
      {[0, 1, 2].map((index) => (
        <motion.span
          ...
          animate={reduce ? undefined : { opacity: [0.28, 1, 0.28], y: [0, -3, 0] }}
...
function pickEmptyPrompt() {
  return emptyPrompts[Math.floor(Math.random() * emptyPrompts.length)] ?? emptyPrompts[0];
}
  ...
  return (
    ...
    >
      ...
      <div className="relative flex flex-col items-center gap-4">
        <span className="flex items-center gap-1.5 text-[#6f8f72]" aria-hidden="true">
          {[0, 1, 2].map((dot) => (
            <motion.span
              key={dot}
              className="size-1.5 rounded-full bg-current"
              initial={reduce ? false : { opacity: 0, scale: 0.6 }}
              animate={reduce ? { opacity: 0.72 } : { opacity: [0.35, 1, 0.55], scale: [0.82, 1.2, 1] }}
              transition={{ duration: 0.95, delay: 0.18 + dot * 0.12, ease: "easeOut" }}
            />
          ))}
        </span>
        <motion.p
  ...
  return (
    <div className="sauna-markdown text-[15px] leading-7 text-[#2f343a]">
      <ReactMarkdown
        ...
        components={{
          p: ({ children }) => <p className="my-3 first:mt-0 last:mb-0 text-pretty">{children}</p>,
          h1: ({ children }) => <h1 className="mb-3 mt-5 text-2xl font-semibold tracking-[-0.05em] text-[#171a1f] first:mt-0">{children}</h1>,
          h2: ({ children }) => <h2 className="mb-3 mt-5 text-xl font-semibold tracking-[-0.04em] text-[#171a1f] first:mt-0">{children}</h2>,
          h3: ({ children }) => <h3 className="mb-2 mt-4 text-lg font-semibold tracking-[-0.03em] text-[#171a1f] first:mt-0">{children}</h3>,
          ul: ({ children }) => <ul className="my-3 list-disc space-y-1.5 pl-5 marker:text-[#6f8f72]">{children}</ul>,
          ol: ({ children }) => <ol className="my-3 list-decimal space-y-1.5 pl-5 marker:font-semibold marker:text-[#6f8f72]">{children}</ol>,
          li: ({ children }) => <li className="pl-1">{children}</li>,
          blockquote: ({ children }) => <blockquote className="my-4 rounded-[20px] border-l-4 border-[#9dbf9b] bg-white/58 px-4 py-3 text-[#4e5660]">{children}</blockquote>,
          code: ({ children, className }) => {
            const isBlock = /language-/.test(className ?? "");
            if (!isBlock) {
              return <code className="rounded-md bg-white/78 px-1.5 py-0.5 font-mono text-[0.92em] text-[#44664d]">{children}</code>;
            }
            return <code className={`${className ?? ""} block overflow-x-auto whitespace-pre p-0 font-mono text-[13px] leading-6 text-[#eef0eb]`}>{children}</code>;
          },
          pre: ({ children }) => <pre className="my-4 overflow-x-auto rounded-[22px] bg-[#171a1f] p-4 shadow-[inset_0_1px_0_rgb(255_255_255_/_0.08)]">{children}</pre>,
          table: ({ children }) => <div className="my-4 overflow-x-auto rounded-[18px] border border-black/[0.08] bg-white/70"><table className="w-full min-w-[420px] border-collapse text-left text-sm">{children}</table></div>,
          th: ({ children }) => <th className="border-b border-black/[0.08] px-3 py-2 font-semibold text-[#171a1f]">{children}</th>,
          td: ({ children }) => <td className="border-b border-black/[0.06] px-3 py-2 align-top text-[#4e5660]">{children}</td>,
          a: ({ children, href }) => <a className="font-semibold text-[#44664d] underline decoration-[#9dbf9b] underline-offset-4 transition hover:text-[#2f5539]" href={href} target="_blank" rel="noreferrer">{children}</a>,
        }}
      >
        {content}
      </ReactMarkdown>
...
function MessageBubble({ message, reduce }: { message: Message; reduce?: boolean | null }) {
  const isUser = message.role === "user";
  const isWorking = message.role === "assistant" && (message.status === "pending" || message.status === "partial") && !message.content.trim();
  return (
    <motion.article
    ...
    >
      {isUser ? (
        <p className="whitespace-pre-wrap text-sm leading-7 text-white/92">{message.content}</p>
      ) : isWorking ? (
        ...
        <MarkdownMessage content={message.content || "正在组织答案..."} />
      )}
    </motion.article>
  );
}

function SubmitGlyph({ busy, reduce }: { busy: boolean; reduce?: boolean | null }) {
  if (!busy) {
...
function ChatMessagesPanel({ messages, statusLabel, busy, focusError, archiveStatus, reduce }: { messages: Message[]; statusLabel: string; busy: boolean; focusError?: string; archiveStatus: string; reduce?: boolean | null }) {
  return (
    <div className="relative flex min-h-0 flex-1 flex-col overflow-hidden px-1 py-5 sm:px-3">
      <div className="relative flex min-h-[420px] flex-1 flex-col overflow-hidden rounded-[34px] border border-black/[0.06] bg-[#fbfbf8] shadow-[inset_0_1px_0_rgb(255_255_255_/_0.78)]">
        <div className="flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          {messages.length ? (
            <div className="flex min-h-full flex-col justify-end gap-4">
              {messages.map((message) => (
                <MessageBubble key={message.id} message={message} reduce={reduce} />
              ))}
              {busy && !messages.some((message) => message.role === "assistant" && (message.status === "pending" || message.status === "partial")) ? (
                <motion.div
        ...
        {focusError ? (
          <div className="mx-4 mb-4 rounded-[24px] bg-[#eadfc8] px-5 py-4 text-sm leading-relaxed text-[#765f34] sm:mx-6">
            <span className="inline-flex items-center gap-2 font-semibold"><WarningCircle size={16} />调用失败</span>
      ...
      <div className="mt-4 rounded-[24px] border border-[#3a2416]/[0.08] bg-[#fbfbf8] p-4 text-xs text-[#68707d]">
        <div className="flex items-center justify-between gap-3">
          <span className="flex items-center gap-2 text-[#44664d]"><CheckCircle size={15} weight="fill" />会话记录</span>
...
export function FocusRoomPanel({ sessionId, initialPrompt = "", draftAgentId = "" }: { sessionId: string; initialPrompt?: string; draftAgentId?: string }) {
  ...
  return (
    <section className="grid min-h-[calc(100dvh-7rem)] gap-5 lg:grid-cols-[320px_minmax(0,1fr)]">
      <motion.aside
        initial={reduce ? false : { opacity: 0, y: 18 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1] }}
        className="rounded-[38px] border border-black/[0.08] bg-[#fbfbf8]/86 p-5 shadow-[0_26px_90px_rgb(28_34_24_/_0.10)] backdrop-blur-xl"
      >
        ...
        <h1 className="mt-6 text-4xl font-semibold tracking-[-0.06em] text-[#171a1f]">{agent?.displayName ?? "智囊"}</h1>
        ...
        <div className="mt-5 rounded-[28px] bg-white/74 p-3 shadow-[0_16px_44px_rgb(28_34_24_/_0.07)]">
          ...
          {historySessions.length ? (
            <div className="mt-2 grid max-h-[330px] gap-2 overflow-y-auto pr-1">
              {historySessions.map((session) => {
                ...
                return (
                  ...
                  >
                    <button type="button" onClick={() => router.push(`/focus-room/${session.id}`)} className="grid size-8 place-items-center rounded-[13px] bg-white text-lg">
                      {session.agentAvatarEmoji || "🧠"}
                    </button>
                    <div className="min-w-0">
                      {editingSessionId === session.id ? (
                        <form onSubmit={submitRename} onClick={(event) => event.stopPropagation()} className="flex gap-1">
                          <input
                            value={editingTitle}
                        ...
                        <span className="flex items-center justify-between gap-2">
                          <span className="truncate text-xs font-semibold text-[#171a1f]">{session.title || session.agentDisplayName}</span>
                          <span className="shrink-0 text-[10px] text-[#87909a]">{formatSessionTime(session.lastActivityAt, hydrated)}</span>
                        </span>
                      )}
                      <span className="mt-0.5 line-clamp-1 block text-[11px] text-[#68707d]">{session.lastMessagePreview || `${session.agentDisplayName || "智囊"} 的会话`}</span>
                    </div>
                    <span className="flex items-center gap-1 self-start">
                      <button type="button" onClick={() => beginRename(session)} className="grid size-7 place-items-center rounded-full bg-white text-[#68707d] transition hover:text-[#171a1f]" aria-label="重命名会话">
                        <PencilSimple size={13} />
                      </button>
                      <button type="button" onClick={() => setPendingDeleteSessionId(session.id)} className="grid size-7 place-items-center rounded-full bg-white text-[#9a4f4f] transition hover:bg-[#eadfc8]" aria-label="删除会话">
                        <Trash size={13} />
                      </button>
                    </span>
                  </div>
            ...
            <p className="mt-2 rounded-[20px] bg-[#f4f5f2] p-3 text-xs leading-relaxed text-[#68707d]">会话会自动保存在这里。</p>
          )}
          <button
            type="button"
      ...
      <motion.div
        ...
        className="relative flex min-h-[680px] flex-col overflow-hidden rounded-[38px] border border-black/[0.08] bg-white/88 p-4 shadow-[0_26px_90px_rgb(28_34_24_/_0.10)] backdrop-blur-xl sm:p-5"
      >
        <div className="relative flex items-center justify-between gap-4 rounded-[30px] bg-[#f4f5f2] p-5">
          <div>
            <h2 className="text-2xl font-semibold tracking-[-0.045em] text-[#171a1f]">{title}</h2>
            <p className="mt-1 text-sm text-[#68707d]">1v1 咨询</p>
          </div>
          <span className={`hidden items-center gap-2 rounded-full px-3 py-1 text-xs font-semibold sm:inline-flex ${statusTone}`}>
            {busy ? <DotsThree size={16} weight="bold" /> : <CheckCircle size={14} weight="fill" />}
            {statusLabel}
          </span>
        </div>
        ...
        <form onSubmit={submitMessage} className="relative flex items-center gap-3 rounded-full border border-black/[0.08] bg-[#f7f8f5] p-2 transition focus-within:border-[#9dbf9b] focus-within:bg-white">
          <input
            value={draft}
            onChange={(event) => setDraft(event.target.value)}
            className="min-w-0 flex-1 bg-transparent px-4 text-sm text-[#171a1f] outline-none placeholder:text-[#87909a]"
            placeholder={token ? (busy ? "智囊正在工作" : "输入问题") : "请先在大厅登录"}
            disabled={!token || busy}
          />
          <button
            type="submit"
